...
2. Хранилище источников и кэша не должно быть жёстко привязано к
   конкретной реализации — старт на файле, с возможностью подключить БД
...
| 5 | Ограничение — хранилище без привязки к реализации | средняя | средняя |
...
| Ограничение — хранилище | Замена хранилища в будущем | Repository pattern поверх хранилища | Domain-слой не знает, в чём именно хранятся источники — файл или PostgreSQL |
...
- **Source Repository** (infrastructure) — CRUD источников/категорий поверх хранилища (файл или PostgreSQL), скрыт за интерфейсом.
...
graph TB
    ...
    SR -->|SOURCE_BACKEND=file| FileDB[(sources.txt<br/>news_cache.json)]
    SR -->|SOURCE_BACKEND=postgres| PgDB[(PostgreSQL:<br/>sources, articles)]
```

## 4. Хранилище: файл и PostgreSQL

Repository pattern (шаги 4–6 выше) позволяет `Source Repository` и кэшу
статей иметь два взаимозаменяемых бэкенда, ни один из которых не виден
`Feed Service` или API-слою:

- **Файл** — `FileSourceRepository` поверх `sources.txt`, кэш статей в
  `news_cache.json`. Используется по умолчанию: для личного проекта на
...
запуска — в [`README.md`](../README.md#хранилище-данных).
