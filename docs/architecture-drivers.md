...
## 4. Обновление после реализации: файл и PostgreSQL вместо SQLite

Шаг 2 (ограничения проектирования) на старте фиксировал SQLite как хранилище.
По факту реализация пошла в два шага, и итоговый выбор — не SQLite:

1. **Первая версия** — `FileSourceRepository` поверх `sources.txt`. Это
   сознательное упрощение: для личного проекта на 1 пользователя файл
   ничем не хуже БД, а Repository pattern (см. шаг 4/6 выше) с самого
   начала гарантировал, что смена хранилища не потребует правок в
   `Feed Service` или API-слое.
2. **Второй бэкенд — PostgreSQL**, а не SQLite. Причина отклонения от
   исходного плана не архитектурная, а практическая: проекту нужна была
   честная возможность попрактиковаться в `EXPLAIN ANALYZE`, индексах и
   работе с полноценной клиент-серверной БД — SQLite для этого не подходит
   (нет `EXPLAIN ANALYZE` в привычном виде, нет отдельного процесса
   сервера). Реализовано как `PostgresSourceRepository` и
   `PostgresArticleCache` — тот же интерфейс, что и у файловых версий.

Оба бэкенда сосуществуют одновременно и переключаются переменной
окружения `SOURCE_BACKEND` (`file` — по умолчанию, `postgres` — БД в
Docker; см. [`README.md`](../README.md#хранилище-данных)). Диаграмма выше
отражает исходный план; фактическая архитектура хранения — это тот же
`Source Repository`/кэш статей, но за интерфейсом может стоять файл
(`sources.txt`, `news_cache.json`) либо PostgreSQL (`db/schema.sql`:
таблицы `sources`, `articles`) — выбор скрыт в `app/dependencies.py`.
